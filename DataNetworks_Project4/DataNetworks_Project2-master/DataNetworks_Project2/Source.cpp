/*
	...
	The server sends data by using a forwarding table defined in the config files.
...
// Legacy, used with flooding algorithm
int maxTimeToLive = 4;
vector<int> framesReceived;

// Store information about neighbors
vector<string> neighborFakeIP;
vector<string> neighborRealIP;
vector<int> neighborPort;

// Variables to store information on where to send data using a forwarding table
int numRouters = 0;
string currentDestination;
map<string, string> forwardTable;
...
string pcapFileName = "Project4input.pcap";
...
///////////////////////////////////////
//
// Description: Forwards data received in the receiver function according to
//				the forwarding table. The portIndexToSendOn is the port to send
//				the data on. Called from the receiver function.
void sender(u_char * dataToSend, int dataSize, int portIndexToSendOn) {
	...
	// create a server socket to the neighbor
	sockaddr_in server;
	server.sin_family = AF_INET; // AF_INET = IPv4 addresses
	server.sin_port = htons(neighborPort[portIndexToSendOn]); // Little to big endian conversion
	...
	// send packet
	int sendOk = sendto(out, (const char *)dataToSend, dataSize, 0, (sockaddr*)&server, sizeof(server));
...
//              If ip doesn't match, use forwarding table to find which server to
//				send the data to.
void receiver() {
	////////////////////////////////////////////////////////////
	// INITIALIZE WINSOCK
	...
	while (true)
	{
		...
		if (bytesIn >= 36) {
			...
			if (frameAlreadyReceived == false) {
				...
				// Check the forwarding table to send the packet
				else {
					...
					// Get the index of the port to send frame on
					int neighborPortIndex = -1;
					if (forwardTable.find(thisFrameDestIP) != forwardTable.end()) {
						for (int i = 0; i < neighborPort.size(); i++) {
							if (neighborFakeIP[i] == forwardTable[thisFrameDestIP]) {
								neighborPortIndex = i;
								break;
							}
						}
					...
					// send the packet
					sender((u_char*)buf, bytesIn, neighborPortIndex);
				}
...
//              If this machines ip matches the ip address, send the data to neighbor
//				specified in the forwarding table.
void initialSender() {
	...
	while (int returnValue = pcap_next_ex(pcap, &header, &pcapData) >= 0)
	{
		...
		string thisFrameDestIP = to_string((int)((unsigned char)dataVector[32])) + "." + to_string((int)((unsigned char)dataVector[33])) + "." +
			to_string((int)((unsigned char)dataVector[34])) + "." + to_string((int)((unsigned char)dataVector[35]));

		map<string, string>::iterator it = forwardTable.find(thisFrameDestIP);
		...
		if (neighborPortIndex != -1) {
			////////////////////////////////////////////////////////////
			// CONNECT TO THE SERVER AND SEND DATA
			////////////////////////////////////////////////////////////

			// create server socket to store on
			sockaddr_in server;
			server.sin_family = AF_INET; // AF_INET = IPv4 addresses
			server.sin_port = htons(neighborPort[neighborPortIndex]); // Little to big endian conversion
			inet_pton(AF_INET, "127.0.0.1", &server.sin_addr); // Convert from string to byte array
			...
			if (thisFrameSourceIP == fakeIP) {
				...
				cout << "Sending frame with destination " << thisFrameDestIP << " on port " << neighborPort[neighborPortIndex] << endl;
	...
	*/
	...
	if (configFile.is_open()) {
		while (!configFile.eof()) {
			...
			else if (configFileIndex <= (2 + (3 * numNeighbors))) {
				// Neighbor fake ip address
				if ((configFileIndex - 3) % 3 == 0) {
					string configOut;
			...
			// get number of routers in network
			else if (configFileIndex == (3 + (3 * numNeighbors))) {
				configFile >> numRouters;
				cout << "NUM ROUTERS: " << numRouters << endl;
			}
			// Get forwarding table
			else {
				// destination ip address
				if ((configFileIndex - (4 + (3 * numNeighbors))) % 2 == 0) {
					string configOut;
					configFile >> configOut;
					currentDestination = configOut;
					cout << "DEST: " << currentDestination;
				}
				// next hop ip address
				else if ((configFileIndex - (4 + (3 * numNeighbors))) % 2 == 1) {
					string configOut;
					configFile >> configOut;
					forwardTable[currentDestination] = configOut;
					cout << " NEXT_HOP: " << configOut << endl;
				}
			}
			configFileIndex++;
		}
